...
from app.models.response_schema import RepositoryDetailsResponse, ProfileResponse, RepositoriesResponse, FollowersResponse, SearchUsersResponse, SearchReposResponse
from app.models.graphql import Variables
from app.models.errors import GitHubAPIError

GITHUB_API_URL = "https://api.github.com"

# max 100 items & expired after 300s
repo_cache: TTLCache[str, RepositoryDetailsResponse | ProfileResponse | RepositoriesResponse | FollowersResponse | SearchUsersResponse | SearchReposResponse | list[Dict[str, Any]]] = TTLCache(maxsize=100, ttl=300)
...
async def graphql_request(query: str, variables: Variables) -> Dict[str, Any]:
    ...
    payload: Dict[str, str | Variables] = {"query": query, "variables": variables if variables else {} }
    async with httpx.AsyncClient() as client:
        response = await client.post(url, json=payload, headers=headers)
        data = response.json()
        return data

async def graphql_search_users(username: str, page: str | None, per_page: int) -> Dict[str, Any]:
    headers = {"Authorization": f"Bearer {GITHUB_TOKEN}"}
    url = f"{GITHUB_API_URL}/graphql"
